...
#pragma warning restore 0219
        ...
        private void RefreshDesiredColumnCount()
        {
            if (!FlowColumnCount.HasValue)
            {
                ...
                if (ItemsSource != null && oldColumnCount != FlowDesiredColumnCount)
                {
                    ...
                    {
                        if (item is FlowGroupColumn internalModel)
        ...
        {
            if (ItemsSource is FlowObservableCollection<object> currentSource && currentSource.Count > 0)
            {
        ...
        {
            if (ItemsSource is FlowObservableCollection<FlowGroup> currentSource && currentSource.Count > 0)
            {
